data <- function(n,p)
{
  ...
  coef_xx = runif(12,1,2.5) 
  
  mat = scale(cbind(g[,1]*e[,1],g[,1]*e[,2],g[,1]*e[,3],g[,2]*e[,4],g[,3]*e[,1],g[,3]*e[,2],
                                       g[,3]*e[,3],g[,4]*e[,4],g[,5]*e[,1],g[,5]*e[,2],g[,5]*e[,3],g[,6]*e[,4]) )
  
  y = c%*%coef_c + e%*%coef_e + g%*%coef_g + mat%*%coef_xx + err   
  
  coef = rep(0,p*q2)
  coef[1]=coef_xx[1]; coef[2]=coef_xx[2]; coef[3]=coef_xx[3]; coef[1*q2+4]=coef_xx[4];
  coef[2*q2+1]=coef_xx[5]; coef[2*q2+2]=coef_xx[6]; coef[2*q2+3]=coef_xx[7]; coef[3*q2+4]=coef_xx[8];
  coef[4*q2+1]=coef_xx[9]; coef[4*q2+2]=coef_xx[10]; coef[4*q2+3]=coef_xx[11]; coef[5*q2+4]=coef_xx[12]; 
