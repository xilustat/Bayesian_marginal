data <- function(n,p)
{
  ...
  {
    g_j = g_norm[,j]
    g_q1=quantile(g_j,1/3); g_q2=quantile(g_j,2/3);
    g_index1=which(g_j<g_q1); g_index2=which(g_q1<=g_j & g_j<g_q2); g_index3=which(g_j>=g_q2)
    g_j[g_index1]=0; g_j[g_index2]=1; g_j[g_index3]=2
  ...
  #y
  err = as.matrix(c(rnorm(0.90*n), 1*rcauchy(0.10*n)))
