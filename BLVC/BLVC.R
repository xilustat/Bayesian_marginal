BLVC = function(e,c,x,w, y, max.steps) 
{
  ...
  for(k in 1:max.steps)
  {
    ...
    invtau2_c = rinvGauss(1, muPrime1, lambdaPrime1)
    ...
    #invtau2_e <- rep(0, q2)
    for (i in 1:q2) {
      invtau2_e[i] <- rinvGauss(1, muPrime2[i], lambdaPrime2)
    }
    ...
    lambda1 <- rgamma(1, shape1, rate1)
    
    lambda2_cSamples[k] <- lambda1
