...
source("data.R")
source("LADBLVC.R")
setwd("/Users/xilu/Desktop/marginal code/LADBLVC")
...
TP_beta2 <- c(TP_beta2,tp(beta_true,beta_med)) 
FP_beta2 <- c(FP_beta2,fp(beta_true,beta_med)) 

TP_beta3 <- c(TP_beta3,tp(beta_true,beta_mode)) 
FP_beta3 <- c(FP_beta3,fp(beta_true,beta_mode)) 

eta_hat <- apply(q_eta, 2, fun)
TP_eta <- c(TP_eta,tp(eta_true,eta_hat)) 
FP_eta <- c(FP_eta,fp(eta_true,eta_hat)) 

TP_eta2 <- c(TP_eta2,tp(eta_true,eta_med)) 
FP_eta2 <- c(FP_eta2,fp(eta_true,eta_med)) 

TP_eta3 <- c(TP_eta3,tp(eta_true,eta_mode)) 
FP_eta3 <- c(FP_eta3,fp(eta_true,eta_mode)) 
...
m21=mean(TP_beta2); s21=sd(TP_beta2); m22=mean(FP_beta2); s22=sd(FP_beta2); 
m23=mean(TP_eta2); s23=sd(TP_eta2); m24=mean(FP_eta2); s24=sd(FP_eta2);

m31=mean(TP_beta3); s31=sd(TP_beta3); m32=mean(FP_beta3); s32=sd(FP_beta3); 
m33=mean(TP_eta3); s33=sd(TP_eta3); m34=mean(FP_eta3); s34=sd(FP_eta3);

save(m1,s1,m2,s2,m3,s3,m4,s4,m21,s21,m22,s22,m23,s23,m24,s24,m31,s31,m32,s32,m33,s33,m34,s34, file = "LADBLVC.RData")
