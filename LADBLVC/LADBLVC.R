LADBLVC = function(e,c,x,w, y, max.steps) 
{
  ...
  for(k in 1:max.steps)
  {
    ...
    s1 = rinvGauss(1, nu=mu_1, lambda=lambda_1)
    
    s1Samples[k,] = s1
    
    #sample s2
    lambda_2 = eta2_2
    mu_2 = sqrt(lambda_2/eta^2)
    index = 1:q2
    flag=1
    while(flag){
      invs = rinvGauss(length(index),nu=mu_2[index],lambda=lambda_2)
      flag = any(invs<=0)|any(is.na(invs))
      s1[index[invs>0]]=1/invs[invs>0]
      index =setdiff(index,index[invs>0]) 
    }
